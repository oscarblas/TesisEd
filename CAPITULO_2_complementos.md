...
**Nota sobre la notación.** En la sección 2.2.3 se introdujeron los horizontes de predicción `N₁, N₂` y el horizonte de control `Nu`, conforme a la formulación general del GPC propuesta por Clarke, Mohtadi y Tuffs [23]. Dado que el sistema hidráulico de cuatro tanques acoplados no presenta tiempo muerto significativo entre la acción de las bombas y la respuesta de los niveles —el flujo se transmite prácticamente al instante a través de las tuberías—, en lo que sigue se adopta la simplificación `d = 0`, lo cual implica `N₁ = 1`. Bajo esta condición, el horizonte de predicción queda completamente caracterizado por su longitud `N ≡ N₂`, y el horizonte de control se denota equivalentemente como `N_u ≡ Nu`. Asimismo, para denotar el número de entradas y salidas físicas del proceso se emplean los símbolos `n_u` y `n_y`, respectivamente —con `n` minúscula para distinguirlos claramente de los horizontes `N` con `N` mayúscula—. Las dimensiones de los vectores y matrices se expresan en la notación estándar `(filas) × (columnas)`, donde el sufijo `× 1` indica un vector columna.
